...
! function (D, u) {
  ...
  function emojiSafeSplit(D, u, F) {
    ...
    for (var C, E, e = [], t = D.length, i = 0; i < t; i++)(55296 <= (E = D.charAt(i)).charCodeAt(0) && E.charCodeAt(0) <= 56319 || 65024 <= D.charCodeAt(i + 1) && D.charCodeAt(i + 1) <= 65039) && (C = ((D.substr(i, 12).split(r) || [])[1] || "").length || 2, E = D.substr(i, C), i += C - (e.emoji = 1)), e.push(">" === E ? "&gt;" : "<" === E ? "&lt;" : E);
    return e
  }
  ...
  var u, a, B = /\s+/g,
    ...
    e = {
      ...
      render: function render(D, u) {
        var F, C, E, e, t, i, n, r, s, a = u.target,
          B = u.prop,
          l = u.text,
          o = u.delimiter,
          A = u.tween,
          ...
          H = A._time,
          I = H - h;
        p && (A._from && (H = A._dur - H), D = 0 === H ? 0 : H < p ? 1e-6 : H === A._dur ? 1 : A._ease((H - p) / (A._dur - p))), D < 0 ? D = 0 : 1 < D && (D = 1), b && (D = 1 - D), F = ~~(D * M + .5), e = D ? ((y < I || I < -y) && (u.setIndex = f = (f + (19 * Math.random() | 0)) % 20, u.chars = c.sets[f], u.prevScrambleTime += I), g) : L.join(o), e = b ? 1 !== D || !A._from && "isFromStart" !== A.data ? (n = l.slice(F).join(o), E = x ? emojiSafeSplit(e).slice(0, d + (j ? 1 - D * D * D : 1) * S - (m ? emojiSafeSplit(n) : n).length + .5 | 0).join("") : e.substr(0, d + (j ? 1 - D * D * D : 1) * S - (m ? emojiSafeSplit(n) : n).length + .5 | 0), n) : (E = "", L.join(o)) : (E = l.slice(0, F).join(o), C = (m ? emojiSafeSplit(E) : E).length, x ? emojiSafeSplit(e).slice(C, d + (j ? 1 - (D = 1 - D) * D * D * D : 1) * S + .5 | 0).join("") : e.substr(C, d + (j ? 1 - (D = 1 - D) * D * D * D : 1) * S - C + .5 | 0)), n = _ ? ((t = (r = b ? w : v) && 0 != F) ? "<span class='" + r + "'>" : "") + E + (t ? "</span>" : "") + ((i = (s = b ? v : w) && F !== M) ? "<span class='" + s + "'>" : "") + o + e + (i ? "</span>" : "") : E + o + e, a[B] = "&nbsp;" === T && ~n.indexOf("  ") ? n.split("  ").join("&nbsp;&nbsp;") : n
      }
    };
